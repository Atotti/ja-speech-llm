...
JOBID=${PBS_JOBID%%.*}
if [ -n "$JOBID" ]; then
  export WANDB_NAME="finetune-adapter-1gpu-${JOBID}"
fi
mkdir -p ./logs
LOGFILE=./logs/finetune-adapter-1gpu-$JOBID.out
ERRFILE=./logs/finetune-adapter-1gpu-$JOBID.err
...
COMMON_ARGS="--max-steps 1000000000 --batch-size 8 --grad-accumulation 16 --warmup-steps 100 --val-check-interval-samples 100000 --lr 1e-4"
