...
if [ -n "$RESUME_FROM" ]; then
    ...
    uv run python -c "from demo2_ja import finetune; finetune(resume_from='${RESUME_FROM}', ${WEIGHTS_ARG} use_text_multiturn=True, lr=${LR_VALUE}, max_steps=1000000000, batch_size=8, grad_accumulation=16, warmup_steps=100, val_check_interval=1000, model_dir='models/v2/LlamaForSpeechLM-ja-Instruct-${TIMESTAMP}')"
elif [ -n "$MODEL_ID" ]; then
    echo "Starting from: $MODEL_ID"
    uv run python -c "from demo2_ja import finetune; finetune(model_id='${MODEL_ID}', ${WEIGHTS_ARG} use_text_multiturn=True, lr=${LR_VALUE}, max_steps=1000000000, batch_size=8, grad_accumulation=16, warmup_steps=100, val_check_interval=1000, model_dir='models/v2/LlamaForSpeechLM-ja-Instruct-${TIMESTAMP}')"
elif [ -n "$ENCODER_ID" ]; then
    echo "Creating fresh model with encoder: $ENCODER_ID"
    uv run python -c "from demo2_ja import finetune; finetune(${ENCODER_ARG} ${WEIGHTS_ARG} use_text_multiturn=True, lr=${LR_VALUE}, max_steps=1000000000, batch_size=8, grad_accumulation=16, warmup_steps=100, val_check_interval=1000, model_dir='models/v2/LlamaForSpeechLM-ja-Instruct-${TIMESTAMP}')"
else
